...
"""
Daily job — pushes a reminder to each call operator who has one or more
Pending or Not Satisfied follow-ups due today or overdue.

Meant to run once per day via a scheduled task (Render Cron Job), since
this is the one notification kind with no natural create/update event
to hook a push send onto.
...
class Command(BaseCommand):
    help = "Sends a push notification to each call operator with due/overdue follow-ups."
    ...
    def handle(self, *args, **options):
        ...
        for employee, followups in by_operator.items():
            ...
            if overdue and due_today:
                ...
                body = (
                    f"{len(overdue)} follow-up"
                    f"{'s' if len(overdue) != 1 else ''} overdue."
                )
            else:
                body = (
                    f"{len(due_today)} follow-up"
                    f"{'s' if len(due_today) != 1 else ''} due today."
                )

            send_push_to_user(
                employee.user,
                title=(
                    f"{len(followups)} follow-up reminder"
                    f"{'s' if len(followups) != 1 else ''}"
                ),
                body=body,
                url="/?page=followups",
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Sent follow-up reminder push to {sent_count} "
                f"operator(s), covering {due.count()} follow-up(s)."
            )
        )
