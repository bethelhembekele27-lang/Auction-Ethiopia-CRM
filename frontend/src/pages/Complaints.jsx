...
export default function Complaints({ complaints, setComplaints, canEdit, addAudit, session }) {
  ...
  async function bulkDelete() {
    ...
    confirm(`Permanently delete ${rows.length} complaint(s)? This cannot be undone.`, async () => {
      setBulkError("");
      try {
        await Promise.all(rows.map((c) => complaintsApi.deleteComplaint(c.id)));
        setComplaints((prev) => prev.filter((c) => !rows.some((r) => r.id === c.id)));
        rows.forEach((c) =>
          addAudit(
            "Delete complaint",
            `${c.id} · ${c.callerName}`,
        ...
        setBulkError(
          err.body?.message ||
            "Couldn't delete one or more complaints — try again."
        );
      }
    });
  }
  ...
  const filtered =
    fStatus === "All"
      ? complaints
      : complaints.filter((c) => c.status === fStatus);
  ...
  function openNew() {
    setEditing(null);
    setDraft({ ...emptyComplaint, date: todayISO() });
    setSaveError("");
    setModalOpen(true);
  }
  ...
  async function save() {
    ...
    if (record.status === "Resolved" && !record.resolutionDate) {
      record.resolutionDate = todayISO();
    }
    ...
    try {
      if (editing) {
        ...
        addAudit(
          ...
          "—",
          `${created.id} created`,
          `${created.category} — ${created.callerName}`
        );
      }
  ...
  async function bulkResolve() {
    ...
    try {
      const updates = await Promise.all(
        rows.map((c) =>
          complaintsApi.updateComplaint(c.id, { status: "Resolved" })
        )
      );
      ...
      setBulkError(
        err.body?.message ||
          "Couldn't resolve one or more complaints — try again."
      );
    }
  }
  ...
  return (
    <div>
      ...
      {bulkError && (
        <div
          className="bg-[color:var(--red-bg)] text-[color:var(--red)] text-[12.5px] px-3 py-2 rounded-md"
      ...
      <Modal
        ...
        title={editing ? `Edit ${editing}` : "New complaint"}
        wide
      >
        <div className="grid grid-cols-2 gap-y-3.5 gap-x-5 mb-2.5">
          <Field label="Related inquiry ID (optional)">
            <input
              className={inputCls}
              value={draft.inquiryId}
              onChange={(e) =>
                setDraft({ ...draft, inquiryId: e.target.value })
              }
            />
          </Field>
          ...
          <Field label="Caller name">
            <input
              ...
              onChange={(e) =>
                setDraft({ ...draft, callerName: e.target.value })
              }
            />
          </Field>
        ...
        <div className="flex flex-wrap gap-2 pt-3.5 border-t border-[color:var(--border)] mt-3.5">
          <button
            className="font-sans text-[13px] font-medium px-3.5 py-2 rounded-[5px] border border-[color:var(--border)] bg-[color:var(--panel)] text-[color:var(--text)] cursor-pointer hover:border-[color:var(--text-3)] bg-[color:var(--brass)] text-white border-[color:var(--brass)]"
            disabled={saving}
            onClick={save}
          >
            {saving
          ...
          <button
            className="font-sans text-[13px] font-medium px-3.5 py-2 rounded-[5px] border border-[color:var(--border)] bg-[color:var(--panel)] text-[color:var(--text)] cursor-pointer hover:border-[color:var(--text-3)] bg-transparent"
